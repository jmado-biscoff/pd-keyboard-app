"""
train_knn_model.py
Trains and evaluates a K-Nearest Neighbors (KNN) model using the preprocessed finger-to-key dataset.
...
print("Training KNN with grid search (this may take a few minutes)...")
...
pkl_path = os.path.join(RESULTS_DIR, "knn_model.pkl")
joblib.dump(best_knn, pkl_path)
...
onnx_path = os.path.join(RESULTS_DIR, "knn_model.onnx")
...
class KNNPyTorchWrapper(torch.nn.Module):
...
knn_torch = KNNPyTorchWrapper(best_knn)
pth_path = os.path.join(RESULTS_DIR, "knn_model.pth")
torch.save(knn_torch, pth_path)
...
# ====================================================
metrics_json = os.path.join(RESULTS_DIR, "knn_metrics.json")
