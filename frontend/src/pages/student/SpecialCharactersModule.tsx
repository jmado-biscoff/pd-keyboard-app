...
import bgImage from "@/assets/b5.jpg";

export default function SpecialCharactersModule() {
  ...
  const getWeakKeys = () => {
    ...
    if (entries.length === 0) return "No key mistakes! Excellent precision!";
    const sorted = entries.sort((a,b)=>b[1]-a[1]);
    const top = sorted.slice(0,3).map(([k])=>k);
    return `You struggled with: ${top.join(", ")}`;
  };
      ...
      <div className="relative z-10 p-8 min-h-screen text-white">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            ...
            <h1 className="font-pixel text-xl text-black">Module 5: Special Characters</h1>
          </div>
          ...
          <PixelCard className="mb-8 bg-black/60 border-[3px] border-yellow-300 backdrop-blur-sm">
            ...
            <input type="text" value={userInput} onChange={(e)=>handleTyping(e.target.value)} onKeyDown={handleKeyDown} disabled={completed}
              className={`w-full px-4 py-3 font-pixel text-sm border-[3px] ${completed?"bg-gray-700 border-gray-500 text-gray-400":"bg-black/70 border-yellow-300 text-white"}`} placeholder={completed?"Module completed!":"Type the symbols above..."} autoFocus/>
          </PixelCard>

          <PixelCard className="mb-8 text-center bg-black/60 border-[3px] border-yellow-300 backdrop-blur-sm">
            <h3 className="font-pixel text-sm mb-4 text-white">Special Character Layout</h3>
            <div className="flex justify-center gap-2">
              {specialLayout[0].map(key=>{
