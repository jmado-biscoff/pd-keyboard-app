...
import bgImage from "@/assets/b5.jpg"; // 🔹 Optional new background

export default function SpecialCharactersModule() {
  const navigate = useNavigate();

  // 🔹 Special Characters Drills
  const lessons = [
    "! @ # $ % ^ & * ( )",
    "!@#$ %^&* ()!@ #$%^",
    "! ! @ @ # # $ $ % %",
  ...
  const getPerformanceMessage = () =>
    wpm >= 40 && accuracy >= 95
      ? "Excellent! You’ve mastered special characters!"
      : wpm >= 25 && accuracy >= 85
      ? "Nice work! You're getting the hang of shifted keys."
      ...
      <div className="relative z-10 p-8 min-h-screen text-white">
        <div className="max-w-6xl mx-auto">
          ...
          <PixelCard className="mb-8 bg-black/60 border-[3px] border-yellow-300 backdrop-blur-sm">
            ...
            <input
              ...
              placeholder={
                completed
                  ? "Module completed!"
                  : `Lesson ${currentIndex + 1}: Type using !@#$%^&*()`
              }
              autoFocus
            />
          </PixelCard>
          ...
          <PixelCard className="mb-8 text-center bg-black/60 border-[3px] border-yellow-300 backdrop-blur-sm">
            <h3 className="font-pixel text-sm mb-4 text-white">Special Characters Layout</h3>
