...
export default function NumberRowModule() {
  ...
  const lessons = [
    ...
    "1 3 5 7 9 0 8 6 4 2",
    "24680 13579 24680 13579",
    "11 22 33 44 55 66 77 88 99 00",
    "1234567890 0987654321",
    "147 258 369 741 852 963",
    "123 321 456 654 789 987",
    "13579 97531 24680 86420",
  ...
  const numberRowLayout = [["1","2","3","4","5","6","7","8","9","0"]];
  ...
  const getWeakKeys = () => {
    ...
    return `Trouble with numbers: ${top.join(", ")}`;
  };

  return (
    <div className="relative min-h-screen overflow-hidden" style={{backgroundImage:`url(${bgImage})`,backgroundSize:"cover",backgroundPosition:"center"}}>
      ...
      <div className="relative z-10 p-8 min-h-screen text-white">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            ...
            <h1 className="font-pixel text-xl text-black">Module 4: Number Row Keys</h1>
          </div>
          ...
          <PixelCard className="mb-8 bg-black/60 border-[3px] border-yellow-300 backdrop-blur-sm">
            ...
            <input type="text" value={userInput} onChange={(e)=>handleTyping(e.target.value)} onKeyDown={handleKeyDown} disabled={completed}
              className={`w-full px-4 py-3 font-pixel text-sm border-[3px] ${completed?"bg-gray-700 border-gray-500 text-gray-400":"bg-black/70 border-yellow-300 text-white"}`} placeholder={completed?"Module completed!":"Type the numbers above..."} autoFocus/>
          </PixelCard>

          <PixelCard className="mb-8 text-center bg-black/60 border-[3px] border-yellow-300 backdrop-blur-sm">
            <h3 className="font-pixel text-sm mb-4 text-white">Number Row Layout</h3>
            <div className="flex justify-center gap-2">
              {numberRowLayout[0].map(key=>{
                const active=key===highlightedKey;
                return(
                  <div key={key} className={`pixel-border w-12 h-12 flex items-center justify-center font-pixel text-sm border-[2px] transition-all duration-150 ${active?"bg-purple-500 text-white scale-110 shadow-[0_0_10px_#a855f7]":"bg-gray-800 text-yellow-100 border-yellow-400"}`}>{key}</div>
                );
              })}
            </div>
          </PixelCard>
          ...
          {completed&&(
            <div className="text-center">
              <PixelCard className="inline-block p-8 bg-black/70 border-[3px] border-yellow-300 backdrop-blur-md">
                ...
                <p className="font-pixel mb-4">{accuracy>=95?"Excellent work!":accuracy>=85?"Nice job! Keep improving!":"Focus on accuracy first."}</p>
